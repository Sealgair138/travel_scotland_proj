...
regional_domestic_tourism <- regional_domestic_tourism %>% 
  #select(-c(measurement, feature_code, breakdown_of_domestic_tourism)) %>% #remove measurement column, date code and feature code
  filter(value != "0")

regional_domestic_tourism1 <- regional_domestic_tourism %>%
  pivot_wider(names_from = units,
              values_from = value) %>%
  clean_names()

regional_domestic_tourism1 <- regional_domestic_tourism1 %>% 
